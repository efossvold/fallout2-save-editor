...
export const saveHandler = (): SaveHandler => {
  ...
  >(
    ...
    if (kind === 'float') {
      ...
      const n = buffer.readIntBE(offset, 0x02)
      const e1 = buffer.readIntBE(offset + 0x02, 0x01)
      const e2 = buffer.readIntBE(offset + 0x03, 0x01)
      return parseFloat(`${n}.${e1}${e2}`) as any
    }
    if (kind === 'int') {
